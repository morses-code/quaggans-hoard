...
async function desktopBoot() {
  ...
  for (const key of ['tyria.defaultCharacter', 'quaggansHoard.protectedItems', 'quaggansHoard.bifrostActive', 'quaggansHoard.selectedLegendary', 'quaggansHoard.trackedLegendary', 'quaggansHoard.trackedRecipe']) {
  ...
  form.onsubmit = async event => {
    ...
    status.textContent = `Checking permissions and saving to ${state.credential_store}…`;
  ...
  window.persistDesktopPreferences = () => {
    ...
    saveQueue = saveQueue.then(() => desktopRequest('/desktop/preferences', preferences)).catch(() => {
      showForm(); status.textContent = 'Your preference could not be saved to disk. Check that your user profile is writable.';
    });
  };
  if (!state.connected) {
    showForm();
