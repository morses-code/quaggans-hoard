...
class Preview(FixtureHandler):
    def do_GET(self):
        ...
        if path == '/app.js':
            ...
            project = b"\nif (location.search.includes('projects')) { showAppView(true); } if (location.search.includes('acquisition')) { selectLegendary(30698); const previewTimer = setInterval(() => { const card = [...document.querySelectorAll('.material-card')].find(node => node.querySelector('strong')?.textContent === 'Mystic Clover'); if (card) card.open = true; if (card?.querySelector('.wiki-acquisition')) { clearInterval(previewTimer); const preview = card.cloneNode(true); preview.querySelectorAll('.source-section-heading').forEach(node => node.parentElement.open = true); document.querySelector('main').hidden = true; document.querySelector('.site-header').hidden = true; preview.style.margin = '18px'; document.body.append(preview); window.scrollTo(0, 0); } }, 100); }"
